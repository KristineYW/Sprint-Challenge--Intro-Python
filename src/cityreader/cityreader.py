...
def cityreader_stretch(lat1, lon1, lat2, lon2, cities=[]):
  ...
  # the specified coordinates.
    ...
    for i in range(len(cities)):
        if (min_lat < cities[i].lat < max_lat) and (min_lon < cities[i].lon < max_lon):
          within.append(cities[i])

    return within
  
lat1 = input('Enter a latitude: ')
lon1 = input('Enter a longitude: ')
lat2 = input('Enter another latitude: ')
lon2 = input('Enter another longitude: ')
